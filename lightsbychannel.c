...
void midi_process(snd_seq_event_t *ev)
{
    ...
    //Note on/off event
       ...
       if( isPlayChannel(ev->data.note.channel) ) {
		  ...
		  // use channel groupings instead of pitch
		  double noteBin;
		  if( data.note.channel == 0 || data.note.channel == 1 ) {
			noteBin = 0;
		  } else if( data.note.channel == 2 || data.note.channel == 3 ) {
			noteBin = 1;
		  } else if( data.note.channel == 4 || data.note.channel == 5 ) {
			noteBin = 2;
		  } else if( data.note.channel == 6 || data.note.channel == 10 ) {
			noteBin = 3;
		  } else if( data.note.channel == 11 || data.note.channel == 12 ) {
			noteBin = 4;
		  } else if( data.note.channel == 7 ) {
			noteBin = 5;
		  } else if( data.note.channel == 8 ) {
			noteBin = 6;
		  } else if( data.note.channel == 9 ) {
			noteBin = 7;
		  }
